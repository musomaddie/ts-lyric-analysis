...
class Word:
    ...
    list approach more as a container as python lists are quite versatile and
    so I have to do less manual work myself.
    ...
    Parameters:
        ...
        is_paragraph_break (bool): is this word a special paragraph break
            character
        duplicates (list<Word): a list containing all the Word objects that are
            the same as this one within the same song.
        parent (Word): initially None, only used if this word is a duplicate of
            another in the same song to link back to it. (TODO: not
